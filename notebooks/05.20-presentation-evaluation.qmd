...
base_blue_1 = "rgb(0, 123, 255)" # Darker blue
base_blue_2 = "rgb(51, 153, 255)" # Slightly lighter
base_blue_3 = "rgb(102, 178, 255)"# Medium blue
base_blue_4 = "rgb(153, 204, 255)"# Lighter
base_blue_5 = "rgb(173, 216, 255)"# Lightest blue

#color_map_plot = { # Used for main prediction plots
#    "Actual (Today's Mean Temp)": "rgb(0, 0, 0)", # black
#    "Train (t2m_1 day(s))": "rgb(170, 219, 35)", "Train (t2m_2 day(s))": "rgb(139, 176, 196)", "Train (t2m_3 day(s))": "rgb(160, 183, 199)",
#    "Train (t2m_4 day(s))": "rgb(181, 205, 210)", "Train (t2m_5 day(s))": "rgb(197, 210, 219)",
...
#    "Test Pred (1 day(s))": "rgb(51, 153, 255)", "Test Pred (2 day(s))": "rgb(102, 204, 255)", "Test Pred (3 day(s))": "rgb(153, 204, 255)",
#    "Test Pred (4 day(s))": "rgb(197, 227, 255)", "Test Pred (5 day(s))": "rgb(224, 242, 255)",
#}
forecast_color_sequence = [
    "rgb(0,   0,   0)",
    "rgb(84, 129, 211)",
    "rgba(255, 90, 90, 0.73)",
    "rgb(84, 129, 211)",
...
#    "Error (1 day(s))": "rgb(0, 0, 255)", "Error (2 day(s))": "rgb(76, 175, 80)", "Error (3 day(s))": "rgb(0, 128, 0)",
#    "Error (4 day(s))": "rgb(255, 193, 7)", "Error (5 day(s))": "rgb(255, 0, 0)",
#}

error_color_sequence = [
    "rgb(231, 107, 119)",
    "rgb(222, 111, 185)",
    "rgb(212, 107, 212)",
    "rgb(206, 105, 210)",
...
## before any figures
#px.defaults.color_discrete_sequence = list(color_map_plot.values())
...
train_context_fig = px.line(
    context_plot_df, x="date", y="t2m", color="dataset", 
    title=f"Actual Mean Temperature for {target_city_name} (Train vs. Test Data)", 
    template="plotly_white",
    color_discrete_map={'Train Data': "rgb(84, 129, 211)", 'Test Data': "rgba(255, 90, 90, 0.73)"} 
)

train_context_fig.update_layout(height=500, xaxis_title="Date", yaxis_title="T2M Mean Temp (°C)") 
...
fig_bare = px.line(plot_df_bare, x="date", y="temperature", color="series", 
                   ...
                   template="plotly_white", color_discrete_sequence=forecast_color_sequence)
...
fig_error_bare = px.line(plot_error_df_bare, x="date", y="error", color="series", 
                         ...
                         template="plotly_white", color_discrete_sequence=error_color_sequence)
...
for i, target_col in enumerate(target_cols_all_horizons): # MODIFIED: Loop through all target horizons
    ...
    importance_df_bare = pd.DataFrame({'Feature': bare_bones_feature_cols, 'Importance': feature_importances})
    ...
    plt.title(f'Top 20 Feature Importances for Bare Bones Model (Target: {target_col})') 
...
fig_full_noncascading = px.line(plot_df_full_noncascading, x="date", y="temperature", color="series", 
                                ...
                                template="plotly_white", color_discrete_sequence=forecast_color_sequence)
fig_full_noncascading.update_layout(
    height=600,
    xaxis_title="Date",
    yaxis_title="T2M Mean Temp (°C)",
    legend=dict(
...
fig_error_full_noncascading.update_layout(
    height=600,
    legend=dict(
        x=1,
        y=1,
...
fig_error_cascading.update_layout(
    height=600,
    legend=dict(
        x=1,
        y=1,
...
for i, target_col in enumerate(target_cols_all_horizons):
    model_for_importance = models_cascading[target_col]
    feature_importances = model_for_importance.feature_importances_
    ...
    importance_df_cascading = pd.DataFrame({'Feature': model_specific_feature_cols_cascading, 'Importance': feature_importances})
...
plot_df_list_native = []
# MODIFIED: Source Actual (Today's Mean Temp) from df_eval_native_multioutput for test period only
plot_df_list_native.append(df_eval_native_multioutput[["date", f"y_true_{target_cols_all_horizons[0]}"]].assign(series="Actual (Today's Mean Temp)").rename(columns={f"y_true_{target_cols_all_horizons[0]}": "temperature"}))
for target_col in target_cols_all_horizons:
    horizon_name = target_col.replace('t2m_', '').replace('_later', ' day(s)')
    # Removed: plot_df_list_native.append(train_df[["date", target_col]].assign(series=f"Train (t2m_{horizon_name})").rename(columns={target_col: "temperature"}))
...
fig_error_native.update_layout(
    height=600,
    legend=dict(
        x=1,
        y=1,
...
## Conclusion
This comparative study of four **XGBoost-based forecasting** strategies for Chattanooga’s daily mean temperature reveals several key insights:

**Feature Engineering Works**: Enhancing the model with derived features, cross-city lags, and rolling statistics significantly improves accuracy over the minimalist baseline. The Bare Bones model had the weakest performance across all metrics.

**Best Overall Accuracy**: The Full Features (Non-Cascading) and Cascading models consistently achieved the lowest RMSE and highest R², **particularly for 1- to 3-day forecasts**. This confirms the value of leveraging historical structure without overly complex modeling strategies.

**Cascading Trade-offs**: While the Cascading approach achieved similar performance to the Non-Cascading version, it introduced **more architectural complexity** and marginal gains beyond 3 days. Its recursive use of predictions introduces some risk of error accumulation, particularly at longer horizons.

**Native Multi-Output**: This model appears to less acurate at the short term but the lookc more robust at the longer dates. You would expect this from a model that accounts for correlations between mutliple outputs. Its **lack of native support for feature importances** and marginal performance differences suggest that its value may lie more in multi-target structure discovery than raw accuracy.

**Generalization Trends**: Across all models, accuracy declines steadily with horizon length, with 5-day forecasts showing ~1.0°C higher MAE than 1-day forecasts. This highlights the growing uncertainty in weather modeling even with extensive feature sets.

# Future Work
To improve forecast accuracy and model robustness, future iterations of this project could include:

**Hyperparameter Optimization:** All models currently use default XGBoost parameters. Techniques such as grid search, randomized search, or Bayesian optimization (e.g., Optuna) may yield substantial improvements.

**Out-of-Fold Validation:** Implement time-aware cross-validation or walk-forward validation to better estimate generalization, particularly for cascading and recursive models.

**Recursive Forecast Refinement:** Expand the Cascading model into a true recursive forecasting framework, where earlier predictions are fed as engineered lag features rather than hard-coded.

**Incorporate Additional Forecast Inputs:**
Soil moisture, solar radiation variability, or NWP-derived indices
Climate anomalies (e.g., ENSO status) for season-aware modeling

**Alternative Algorithms:** Benchmark performance against other temporal models such as LightGBM, CatBoost, and neural networks (LSTMs, TCNs, or Transformers).

**Uncertainty Quantification:** Add prediction intervals using quantile regression, bootstrapping, or ensemble variance to improve decision-making utility.


```{python}
# Legend label:
series_label = "Test Pred (1 day(s))"

# Your map key:
map_key     = "Test Pred (1 day(s))"

print(series_label == map_key)     # must be True
print(repr(series_label), repr(map_key))
