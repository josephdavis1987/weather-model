---
title: "03.61 – XGBoost Multi output, Spatiotemporal Features with 3 surrounding cities"
format: html
jupyter: python3
----------------
...
for other_city in surrounding_cities:
    ...
    for feature in cross_city_features_to_lag:
        for lag in cross_city_lags:
            ...
            lagged_data['date'] = lagged_data['date'] + pd.Timedelta(days=lag) # Shift date forward to align for merge
...
fig = px.line(
    ...
    y="temperature",
    color="series",
    title=f"XGBoost Baseline for {target_city_name} - Multi-Day Forecasts (Target: T2M)", # Updated title
...
fig_error = px.line(
    ...
    y="error",
    color="series",
    title=f"Forecast Error for {target_city_name} by Horizon (Test Set - Target: T2M)", # Updated title
