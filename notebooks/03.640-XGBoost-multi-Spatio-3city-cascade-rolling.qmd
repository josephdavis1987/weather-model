---
title: "03.64 – XGBoost Cascading Multi-Output Forecasts (1-5 Days, with Rolling Features)"
format: html
jupyter: python3
----------------
...
rolling_features_to_calculate = [
    "t2m", "dewpoint", "ps", "prectotcorr", "t2m_avg_from_max_min" # Added t2m_avg_from_max_min
]
...
for feature in rolling_features_to_calculate:
    for window in rolling_windows:
        rolling_series = df[feature].rolling(window=window, min_periods=1) # min_periods avoids too many NaNs
...
# Collect all relevant columns for dropna: targets + all base/derived + all intra-city lags + all cross-city lags + all rolling features
all_cols_to_check_for_nan = list(target_cols) 
all_cols_to_check_for_nan.extend(initial_feature_cols) # This now includes all types of features
